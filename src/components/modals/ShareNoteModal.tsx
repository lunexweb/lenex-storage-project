...
export default function ShareNoteModal({
  ...
  const whatsappMessage = generated
    ? `You have been sent a note to view from ${businessName}.\n\nOpen the link below and enter the code to view it (view only).\n\n<${requestLink}>\n\nCode: ${generated.code}\n\nNote: ${noteTitle}`
    : "";
  const emailSubject = `View note from ${businessName}`;
      ...
      <div className="relative bg-card rounded-2xl shadow-xl border border-border w-full max-w-md max-h-[90vh] flex flex-col animate-scale-in">
        <div className="flex items-center justify-between px-6 py-4 border-b border-border shrink-0">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              Share this note via link and code. Recipient can view only (no edit).
            </p>
          </div>
        ...
        <div className="p-6 space-y-4 overflow-y-auto min-h-0 flex-1">
          <div className="bg-muted/50 border border-border rounded-lg px-4 py-3 text-sm">
            ...
            <strong className="text-foreground truncate block">{noteTitle}</strong>
          </div>
